...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function _addLiquidity(
        ...
        require(pair != address(0), "UniswapV2Router: PAIR_CREATION_FAILED");
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to;
            if (i < path.length - 2) {
                address pairNext = IUniswapV2Factory(factory).getPair(output, path[i + 2]);
                require(pairNext != address(0), "UniswapV2Router: PAIR_NOT_FOUND");
                to = pairNext;
            } else {
                to = _to;
            }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to;
            if (i < path.length - 2) {
                to = IUniswapV2Factory(factory).getPair(output, path[i + 2]);
                require(to != address(0), "UniswapV2Router: PAIR_NOT_FOUND");
            } else {
                to = _to;
            }
